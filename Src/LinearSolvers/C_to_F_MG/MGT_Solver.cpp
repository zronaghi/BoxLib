...
//#endif
  for ( int lev = 0; lev < m_nlevel; ++lev ) {
    for (MFIter mfi(*(sig[lev]),false); mfi.isValid(); ++mfi)
    {
...
//#endif
  ...
  {
    const FabSet& fs = bd.bndryValues(oitr());
    for (MFIter umfi(*(uu[lev]),false); umfi.isValid(); ++umfi)
    {
      FArrayBox& dest = (*(uu[lev]))[umfi];
      dest.copy(fs[umfi],fs[umfi].box());
    }
...
//#endif
  {
    ...
    {
      if (rhnd.defined(lev)) {
        for (MFIter mfi(rhnd[lev], false); mfi.isValid(); ++mfi) 
        {
          const int n = mfi.LocalIndex();
          const Box& bx = mfi.tilebox();
    ...
    if (have_rhcc) {
      for ( int lev = 0; lev < m_nlevel; ++lev ) {
        for (MFIter mfi(*(rhcc[lev]), false); mfi.isValid(); ++mfi) 
        {
          const int n = mfi.LocalIndex();
          const Box& bx = mfi.tilebox();
...
//#endif
  for ( int lev = 0; lev < m_nlevel; ++lev )
  {
    ...
    for (MFIter mfi(*(vel[lev]), false); mfi.isValid(); ++mfi)
    {
      const int n = mfi.LocalIndex();
      const Box& ccbx = mfi.growntilebox(1);
  ...
  MGT_Solver::set_cfa (const MultiFab& aa, int lev)
{
  ...
  for (MFIter mfi(aa, false); mfi.isValid(); ++mfi)
  {
    const int n = mfi.LocalIndex();
    const Box& bx = mfi.tilebox();
...
{
  // the caller has started OMP
  for (MFIter mfi(bb,false); mfi.isValid(); ++mfi)
  {
    const int n = mfi.LocalIndex();
    const Box& bx = mfi.tilebox();
