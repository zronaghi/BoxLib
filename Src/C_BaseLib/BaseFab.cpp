                     ...
                     int        numcomp) const
{
    ...
    if (p == 0)
    {
        ...
        Kokkos::parallel_reduce(mdpolicy<4>({lo[0], lo[1], lo[2], comp}, {hi[0]+1, hi[1]+1, hi[2]+1, comp+numcomp}, {cb[0], cb[1], cb[2], comp+numcomp}), 
            P0NormFunctor<Real>(this->view_fab), nrm);
#else
#error "BaseFab::norm needs to be implemented for other spacedims"
    ...
    else if (p == 1)
    {
        ...
        Kokkos::parallel_reduce(mdpolicy<4>({lo[0], lo[1], lo[2], comp}, {hi[0]+1, hi[1]+1, hi[2]+1, comp+numcomp}, {cb[0], cb[1], cb[2], comp+numcomp}), 
            P1NormFunctor<Real>(this->view_fab), nrm);
#else
#error "BaseFab::norm needs to be implemented for other spacedims"
#endif
    }
...
//	nrm = fort_fab_norm(ARLIM_3D(bx.loVect()), ARLIM_3D(bx.hiVect()),
   //			    BL_TO_FORTRAN_N_3D(*this,comp), &numcomp,
   //			    &p);
//    }
//    else
//    {
                    ...
                    int        ncomp) const
{
    ...
    Kokkos::parallel_reduce(mdpolicy<4>({lo[0], lo[1], lo[2], comp}, {hi[0]+1, hi[1]+1, hi[2]+1, comp+ncomp}, {cb[0], cb[1], cb[2], comp+ncomp}), 
